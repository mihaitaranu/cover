...
jQuery.fn.MediaManager = function( options ) {
	...
	return this.each( function( i ) {		  
		...
		function _mm_editor_save() {
			...
			/* Image cover */
			if ( $( '#mm-editor-content .image-holder-cover > img' ).length && $( '#mm-editor-content .track-cover' ).attr( 'type' ) == 'hidden' ) {

				$audio_cover = $( '#mm-editor-content .image-holder-cover > img' ).attr('src');
				$( '#'+item_id, mm ).find( '.mm-item-preview img' ).addClass('mm-audio-custom-cover').attr( 'src', $audio_cover );
			} else {
				$placeholder = $( '#mm-editor-content .image-holder-cover' ).data('placeholder');
				$( '#'+item_id, mm ).find( '.mm-item-preview img' ).removeClass('mm-audio-custom-cover').attr( 'src', $placeholder );
			}
