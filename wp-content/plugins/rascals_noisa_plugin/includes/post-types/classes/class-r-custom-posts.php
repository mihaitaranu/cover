...
class R_Custom_Post {
	...
	function __construct( $args, $options ) {
		...
		if ( $this->args['admin_path'] == '' ) {
			...
			$_path = $this->args['admin_path'];
		}
		$this->admin_path = $_path . $this->args['admin_dir'];

		/* Set URI path */
		if ( $this->args['admin_dir'] == '' ) {
			$_path_uri = get_template_directory();
		} else {
			$_path_uri = $this->args['admin_dir'];
		}
	...
	function add_admin_menu() {	
		$panel_sub_page = add_submenu_page( 'edit.php?post_type=' . $this->args['post_name'], $this->args['post_name'] . '_sort', __( 'Sort Items', 'custom_post_class' ), 'moderate_comments', $this->args['post_name'], array(&$this, 'init'));
	...
	function display() {
		...
		echo '<h3>' . __( 'Sort Items', 'custom_post_class' ) . ' <img src="' . site_url() . '/wp-admin/images/loading.gif" id="loading-animation" alt="' . $this->args['post_name'] . '"/></h3>';
		...
		while ( $sort_query->have_posts() ){ 
			...
			echo '<a href="' . home_url() . '/wp-admin/post.php?action=edit&post=' . get_the_ID() . '" class="edit-item" title="' . __( 'Edit This Post', 'custom_post_class' ) . '"></a>';
