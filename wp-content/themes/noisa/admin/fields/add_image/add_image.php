		...
		public function render() {
			...
			if ( isset( self::$_option['button_title'] ) && self::$_option['button_title'] ) { 
				$button_title = self::$_option['button_title'];
			 	...
			 	$button_title = __( 'Add File', 'noisa' );
			}
			...
			if ( isset( self::$_option['msg'] ) && self::$_option['msg'] ) { 
				$msg = self::$_option['msg'];
			 	...
			 	$msg = __( 'Currently you don\'t have images, you can add them by clicking on the button below.', 'noisa' );
			}
			...
			if ( isset( self::$_option['std'] ) && self::$_option['std'] != '' ) {
					...
					// If image exists
					if ( $image ) {
						echo '<img src="' . $image . '" alt="' . __( 'Preview Image', 'noisa' ) . '">';
					} else {
						$display = 'block';
						$del_display = 'none';
					}
			...
			echo '<p class="msg msg-error" style="display:none">' . __( 'The link is incorrect or the image does not exist.', 'noisa' ) . '</p>';
				
			// Button
			echo '<button class="_button upload-image" style="display:' . $display . '"><i class="fa icon fa-plus"></i>' . $button_title . '</button>';

			echo '<button class="_button ui-button-delete delete-image" style="display:' . $del_display . '"><i class="fa icon fa-trash-o"></i>' . __( 'Remove', 'noisa' ) . '</button>';
