	...
	class MuttleyPanel_bg_generator extends MuttleyPanel {
        ...
        */
		public function render() {
			...
			echo '<button class="_button generate-bg" style="' . $display_g . '"><i class="fa icon fa-magic"></i>' . __('Generate Background', 'noisa' ) . '</button>';
			
			echo '<button class="_button ui-button-delete delete-bg" style="' . $display_d . '"><i class="fa icon fa-trash-o"></i>' . __('Remove', 'noisa' ) . '</button>';
		...
		private function widget() {
			...
			echo '<div class="dialog-row">
			      <label for="bg-type">' . __('Background Type', 'noisa' ) . '</label>
		          ...
		          <p class="help-box">' . __('Select background type.', 'noisa' ) . '</p>
				  </div>';
			...
			<label for="bg-color">' . __('Color', 'noisa' ) . '</label>
			<label for="bg-color-transparent" class="checkbox-label">' . __('Transparent: ', 'noisa' ) . ' <input type="checkbox" id="bg-color-transparent" name="bg_color_transparent" value="" /></label>
			<div class="clear"></div>
			<input type="text" id="bg-color" name="bg_color" value="#ffffff" class="colorpicker-input"/>
			<div class="clear"></div>
			<p class="help-box">' . __('Select background color.', 'noisa' ) . '</p>
			</div>
			</div>';
			...
			<div class="dialog-row">
			<label for="bg-file">' . __('File URL ', 'noisa' ) . '</label>
			...
			<p class="msg msg-error" style="display:none">' . __( 'The link is incorrect or the image does not exist.', 'noisa' ) . '</p>

			<button class="_button upload-image"><i class="fa icon fa-plus"></i>' . __( 'Add File', 'noisa' ) . '</button>
			<button class="_button ui-button-delete delete-image" style="display:none"><i class="fa icon fa-trash-o"></i>' . __( 'Remove', 'noisa' ) . '</button>
			<img class="ajax-loader" src="' . esc_url(admin_url('images/wpspin_light.gif')) . '" alt="Loading..." style="display:none" />
			<p class="help-box">' . __('Enter image URL for your background.', 'noisa' ) . '</p>
			</div>
			<div class="dialog-row">
			<label for="bg-pos">' . __('Position', 'noisa' ) . '</label>
		          ...
		          <p class="help-box">' . __('The first value is the horizontal position and the second value is the vertical. The top left corner is 0 0. Units can be pixels (0px 0px) or any other CSS units. If you specify only one value, the other value will be 50%. You can mix % and positions', 'noisa' ) . '</p>
			</div>
			
			<div class="dialog-row custom-pos-group" style="display:none">
			<label for="bg-custom-pos">' . __('Custom', 'noisa' ) . '</label>
			<input type="text" id="bg-custom-pos" name="bg_custom_pos" value="50% 50%" class="image-input"/>
			</div>
			
			<div class="dialog-row">
			<label for="bg-repeat">' . __('Repeat', 'noisa' ) . '</label>
		          ...
		          <p class="help-box">' . __('The background-repeat property sets if/how a background image will be repeated.', 'noisa' ) . '</p>
			</div>
			
			<div class="dialog-row">
			<label for="bg-att">' . __('Attachment', 'noisa' ) . '</label>
			      <select name="bg_attt" id="bg-att" size="1" class="bg-att">
			      <option value="scroll">scroll</option>
				  <option value="fixed">fixed</option>
				  </select>
		          <p class="help-box">' . __('The background-attachment property sets whether a background image is fixed or scrolls with the rest of the page.', 'noisa' ) . '</p>
			</div>
